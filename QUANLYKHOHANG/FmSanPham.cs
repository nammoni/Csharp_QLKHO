...
namespace QUANLYKHOHANG
{
    public partial class FmSanPham : Form
    {
        ...
        private void DataSanPham_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            cmMaKho.Text = DataSanPham.SelectedRows[0].Cells[2].Value.ToString();
            txtTenSp.Text = DataSanPham.SelectedRows[0].Cells[3].Value.ToString();
            ...
            cmMaLSp.Text = DataSanPham.SelectedRows[0].Cells[7].Value.ToString();
            txtGiaSp.Text = DataSanPham.SelectedRows[0].Cells[8].Value.ToString();
            txtGhiChuSp.Text = DataSanPham.SelectedRows[0].Cells[9].Value.ToString();
        ...
        public bool KTThongTin()
        {
            ...
            if (cmMaNCC.Text == "")
            {
                MessageBox.Show("Vui lòng chọn Mã nhà cung cấp", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                cmMaNCC.Focus();
                return false;
            }
            if (txtTenSp.Text == "")
            ...
            if (cmMaLSp.Text == "")
            {
                MessageBox.Show("Vui lòng chọn Mã Loại sản phẩm", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                cmMaLSp.Focus();
                return false;
            }
            if (txtSoLuong.Text == "")
            ...
            if (cmMaKho.Text == "")
            {
                MessageBox.Show("Vui lòng chọn Mã Kho", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                cmMaKho.Focus();
                return false;
            }
            return true;
        ...
        private void btnThemSp_Click(object sender, EventArgs e)
        {
            ...
            if (KTThongTin())
            {
                ...
                if (MessageBox.Show("Bạn có chắc chắn thêm không?", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("INSERT_SAN_PHAM", _cnn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@MA_SP", SqlDbType.Char).Value = txtMaSp.Text;
                        cmd.Parameters.AddWithValue("@MANCC", SqlDbType.Char).Value = cmMaNCC.Text;
                        cmd.Parameters.AddWithValue("@TEN_SP", SqlDbType.NVarChar).Value = txtTenSp.Text;
                        cmd.Parameters.AddWithValue("@NGAYSX", SqlDbType.Date).Value = dateNSX.Value;
                        cmd.Parameters.AddWithValue("@HSD", SqlDbType.Date).Value = dateHSD.Value;
                        cmd.Parameters.AddWithValue("@SOLUONG_SP", SqlDbType.Int).Value = txtSoLuong.Text;
                        cmd.Parameters.AddWithValue("@MALOAI", SqlDbType.Char).Value = cmMaLSp.Text;
                        cmd.Parameters.AddWithValue("@GIA", SqlDbType.Float).Value = txtGiaSp.Text;
                        cmd.Parameters.AddWithValue("@GHICHU_SP", SqlDbType.NVarChar).Value = txtGhiChuSp.Text;
                        cmd.Parameters.AddWithValue("@MAKHO", SqlDbType.Char).Value = cmMaKho.Text;
                        cmd.Parameters.AddWithValue("@ANH", SqlDbType.Image).Value = ImageToByArray(pictureBox1);
                        cmd.ExecuteNonQuery();
                        LoadDulieu();
        ...
        private void btnSuaSP_Click(object sender, EventArgs e)
        {
            if (KTThongTin())
            {
                if (MessageBox.Show("Bạn có chắc chắn Sửa không?", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("UPDATE_SAN_PHAM", _cnn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@MA_SP", SqlDbType.Char).Value = txtMaSp.Text;
                        cmd.Parameters.Add("@MANCC", SqlDbType.Char).Value = cmMaNCC.Text;
                        cmd.Parameters.Add("@TEN_SP", SqlDbType.NVarChar).Value = txtTenSp.Text;
                        cmd.Parameters.Add("@NGAYSX", SqlDbType.Date).Value = dateNSX.Value;
                        cmd.Parameters.Add("@HSD", SqlDbType.Date).Value = dateHSD.Value;
                        cmd.Parameters.Add("@SOLUONG_SP", SqlDbType.Int).Value = txtSoLuong.Text;
                        cmd.Parameters.Add("@MALOAI", SqlDbType.Char).Value = cmMaLSp.Text;
                        cmd.Parameters.Add("@GIA", SqlDbType.Float).Value = txtGiaSp.Text;
                        cmd.Parameters.Add("@GHICHU_SP", SqlDbType.NVarChar).Value = txtGhiChuSp.Text;
                        cmd.Parameters.Add("@MAKHO", SqlDbType.Char).Value = cmMaKho.Text;
                        cmd.Parameters.Add("@ANH", SqlDbType.Image).Value= ImageToByArray(pictureBox1);
                        cmd.ExecuteNonQuery();
                        LoadDulieu();
        ...
        private void btnXoaSp_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc chắn Xóa không?", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                try
                {
                    SqlCommand cmd = new SqlCommand("DELETE_SAN_PHAM", _cnn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@MA_SP", SqlDbType.Char).Value = txtMaSp.Text;
                    cmd.ExecuteNonQuery();
                    LoadDulieu();
